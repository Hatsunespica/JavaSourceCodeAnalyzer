...
public class NumberOfAddedCalculator  extends InheritanceMetricCalculator {
        ...
        for(Map.Entry<Long,List<Long>> entry:inheritanceTree.entrySet()) {
            ...
            for (Long son : entry.getValue()) {
                result.put(className.get(son).getFullName(),
                        className.get(son).getMethods().size() - ClassData.intersect(className.get(son).getMethods(), className.get(entry.getKey()).getMethods()));
            }
        }
        return result;
    }
